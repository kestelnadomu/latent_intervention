...
| symbol | meaning |
|---|---|
| $z \in \mathcal Z = \mathbb R^{128}$ | frozen representation from the configured LangVAE or Nomic encoder |
| $\mathbf s \in \mathcal S$ | structured state in schema order; the active talent schema is $(s_X,s_T,s_D,s_U)$ with $\lvert\mathcal S\rvert = 4\cdot3\cdot3\cdot3 = 108$ |
